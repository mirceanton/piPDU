...
def get_expanders():
    if len(EXPANDER_I2C_BUS) < 1:
        raise ValueError("""
            At least 1 i2c bus must be speciffied!
    ...
    if len(EXPANDER_I2C_BUS) > 1 and len(EXPANDER_I2C_BUS) != len(EXPANDER_I2C_ADDR):
        raise ValueError("""
          If more than 1 i2c bus is specified, there should be an i2c bus per i2c address.
          EXPANDER_I2C_BUS and EXPANDER_I2C_ADDR must have the same number of elements.
        """.strip())
    ...
    # Otherwise, use the specified i2c bus for the expander
